...
tag in use (across raw/ and findings/) against the canonical glossary in
research/findings/tags.md.

Per the plan's key decisions: this file is generated, not hand-maintained. It should be small
enough to fit entirely in an agent's context, so most "have we looked at X" queries can be
answered from this file plus one or two findings/ files, without a full-repo search.

Requires: pip install python-frontmatter

Usage:
    python build_index.py            # regenerate research/_index.md
    python build_index.py --check    # exit 1 if _index.md is out of date or any tag is
                                      # undefined, without writing anything (for CI / pre-commit)
"""
import argparse
import glob
import re
...
def validate_tags(glossary):
    """Check every tag used in raw/ and findings/ frontmatter against the glossary.
    Returns a list of (file, tag) problems."""
    problems = []
    all_files = (
...
def build_index_content(findings, sessions):
    ...
    lines = [
        ...
        "last updated → backing raw sessions. Small enough to fit in an agent's context in full, "
        "so most \"have we looked at X\" queries should be answerable from this file plus one or "
        "two `findings/` files, without a full-repo search.\n",
        "| Topic | Tags | Related Components | Last Updated | Backing Raw Sessions |",
        "|---|---|---|---|---|",
    ]
    for r in rows:
        lines.append(f"| [{r['topic']}]({r['file']}) | {r['tags']} | {r['components']} | {r['updated']} | {r['raw']} |")
    return "\n".join(lines) + "\n"
...
def main():
    ...
    parser.add_argument("--check", action="store_true", help="Exit 1 if index is stale or tags/links are invalid; don't write")
    ...
    new_content = build_index_content(findings, sessions)
    old_content = INDEX_FILE.read_text(encoding="utf-8") if INDEX_FILE.exists() else None
    ...
    if link_problems:
        print("⚠️  Cross-reference issues:", file=sys.stderr)
